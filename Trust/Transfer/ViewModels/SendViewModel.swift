...
struct SendViewModel {
    ...
    func currentPairPrice() -> Decimal? {
        guard let currentTokenInfo = storage.coinTicker(by: destinationAddress), let price = Decimal(string: currentTokenInfo.price) else {
            return nil
        }
        return price
    }
    ...
    func isFiatViewHidden() -> Bool {
        guard let currentTokenInfo = storage.coinTicker(by: destinationAddress), let price = Decimal(string: currentTokenInfo.price), price > 0 else {
            return true
        }
        return false
    }
