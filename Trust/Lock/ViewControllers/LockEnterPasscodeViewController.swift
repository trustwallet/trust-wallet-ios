...
class LockEnterPasscodeViewController: LockPasscodeViewController {
    ...
    override func enteredPasscode(_ passcode: String) {
        ...
        if lock.isPasscodeValid(passcode: passcode) {
            ...
            let text = String(format: NSLocalizedString("lock.enter.passcode.view.model.incorrect.passcode.text", value: "Incorrect passcode. You have %d attempts left.", comment: ""), passcodeAttemptLimit - numberOfAttempts)
